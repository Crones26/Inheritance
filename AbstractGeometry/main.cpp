...
#define SHAPE_GIVE_PARAMETERS start_x, start_y, line_width, color
	...
	public:
		...
		void draw() const override
		{
			...
			HPEN hPen = CreatePen(PS_SOLID, line_width, color);// создает сплошное перо шириной 5 пикселей и заданного цвета (color).
			HBRUSH hBrush = CreateSolidBrush(color); // создает кисть заданного цвета.
			//выбирает созданные перо и кисть в контекст устройства.
			SelectObject(hdc, hPen);
			SelectObject(hdc, hBrush);
			// задает координаты вершин треугольника с учетом смещений 
			POINT points[3] = { {start_x, start_y + 100}, {start_x + 100, start_y}, {start_x + 200, start_y + 100} };
...
void main()
{
	...
	Geometry::Triangle triangle(50, 60, 70, 300, 200, 5, Geometry::Color::GREEN);
	triangle.info();
